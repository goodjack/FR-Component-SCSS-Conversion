...
const ThemesPage = React.createClass({
  ...
  getInitialState() {
    return {
      ...
      muiConfig: this.context.muiConfig ? this.context.muiConfig : ConfigurationManager.getMuiConfig(SiteBConfig),
      compTheme: this.context.compTheme ? this.context.compTheme : ThemeManager.getCompTheme(DarkRawTheme),
      compConfig: this.context.compConfig ? this.context.compConfig : ConfigurationManager.getCompConfig(SiteBConfig),
      isThemeDark: false,
      isSiteBConfig: false,
      dialogOpen: false,
      snackbarOpen: false,
  ...
  render() {
    ...
    return (
      <div>
        <h2 style={styles.headline}>Site B Configuration</h2>
        <Grid maxCols={1}>
          <GridCell style={{ background: '#ccc', lineHeight: '20px', color: '#333', fontSize: '0.7em', textAlign: 'center' }}>
            Configuration changed to set 'maxCols = 20'.
          </GridCell>
        </Grid>
        <Grid>
